...
def BigGan(sn=False):
    ...
    z = K.Input(shape=[160], dtype=tf.float32,
        name='z') 
    ...
    z_splits = K.layers.Lambda(lambda z: tf.split(z, 8, axis=1),
        name='split')(z)
    ...
    net = Block(net, z_splits[2], embeded_c, 96*8, 
        name='block_2') 
    net = Block(net, z_splits[3], embeded_c, 96*8, 
        name='block_3') 
    net = Block(net, z_splits[4], embeded_c, 96*4, 
        name='block_4') 
    net = O.layers.SelfAttention(filters=96*2, reduction=8, sn=sn, 
        name='attention')(net) 
    net = Block(net, z_splits[5], embeded_c, 96*2, 
        name='block_5') 
    net = Block(net, z_splits[6], embeded_c, 96*1, 
        name='block_6') 
    net = Block(net, z_splits[7], embeded_c, 96*1, 
        name='block_7') 
